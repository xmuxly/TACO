...
class Oxford(data.Dataset):
    def __init__(self, data_path, split='train', real=False, vo_lib='stereo', num_grid_x=1, num_grid_y=3, block_num=1, augment=False):
        ...
        for seq in seqs:
            ...
            # pcs_all.extend([osp.join(seq_dir, 'sphere_velodyne_left_seg', '{:d}.bin'.format(t)) for t in ts[seq]])
    ...
    def __getitem__(self, index):
        ...
        # with mask
        # ptcld = load_velodyne_binary_seg_feature32(scan_path)  # (N, 4)
        ...
        rot = self.rots[index]
        # ground truth
        gt = (rot @ xyz.transpose()).transpose() + pose[:3].reshape(1, 3)
        if self.augment:
            thera = random.randint(1, 360)
            if thera > 180:
                thera = 180
            else:
                thera = 0
            sin_value = math.sin(math.radians(thera))
            cos_value = math.cos(math.radians(thera))
            matrix = np.zeros((3, 3))
            ...
            rot = np.dot(matrix, rot)
            gt = (rot @ xyz.transpose()).transpose() + pose[:3].reshape(1, 3)
            xyz = np.dot(matrix, xyz.transpose()).transpose()
