...
def collate_fn_default(data):
    ...
    for i in range(batch_size):
        b_idx.append(torch.ones(point_num[i]) * i) # 为每个点添加索引值
    points = [torch.from_numpy(d['point_feat']) for d in data] # 从字典中提取特征、标签、位置，从numpy数组转换为pytorch张量
    labels = [torch.from_numpy(d['point_label']) for d in data]
    pose = [torch.from_numpy(d['pose']) for d in data]
    rot = [torch.from_numpy(d['rot']) for d in data]

    max_gt = max([len(d['gt_boxes']) for d in data])
    batch_gt_boxes3d = np.zeros((batch_size, max_gt, data[0]['gt_boxes'].shape[-1]), dtype=np.float32)
    for k in range(batch_size):
        batch_gt_boxes3d[k, :data[k]['gt_boxes'].__len__(), :] = data[k]['gt_boxes']
